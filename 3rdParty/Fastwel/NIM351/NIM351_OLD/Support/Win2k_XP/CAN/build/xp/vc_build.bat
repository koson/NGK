...
if not exist "..\..\configs\xp\%1.cfg" echo Build configuration file "..\..\configs\xp\%1.cfg" not found & goto build_failed

echo ===================================================================
echo Building configuration: %1
echo ===================================================================

if exist "..\..\bin\xp\%1.exe" del "..\..\bin\xp\%1.exe"
...
set f_vc_cfg = ""
if "%f_vc_ver%"=="90" set f_vc_cfg="vc2008_xp.cfg" & goto tool_selected
if "%f_vc_ver%"=="80" set f_vc_cfg="vc2008_xp.cfg" & goto tool_selected
if "%f_vc_ver%"=="71" set f_vc_cfg="vc2003_xp.cfg" & goto tool_selected
if "%f_vc_ver%"=="70" set f_vc_cfg="vc2003_xp.cfg" & goto tool_selected
if "%f_vc_ver%"=="60" set f_vc_cfg="vc6_xp.cfg" & goto tool_selected
...
findstr /b "COMPILE=" "..\..\configs\xp\%1.cfg" > cluopt.tmp
if "%errorlevel%" neq "0" echo Build configuration file (..\..\configs\xp\%1.cfg) doesn't consist of user options COMPILE= & goto build_failed
for /f "tokens=2* delims==" %%A in (cluopt.tmp) do (echo %%A >> "..\..\intdir\ccmpl.rsp")

rem Get system options for compiler
findstr /b "COMPILE=" %f_vc_cfg% > clsopt.tmp
if "%errorlevel%" neq "0" echo System config file (%f_vc_cfg%) doesn't consist of options for COMPILE= & goto build_failed
for /f "tokens=2* delims==" %%A in (clsopt.tmp) do (echo %%A >> "..\..\intdir\ccmpl.rsp")
...
rem echo /LIBPATH:"%CESDK%\Lib\x86" >> "..\..\intdir\clnk.rsp" 
...
findstr /b "LINK=" %f_vc_cfg% > lnksopt.tmp
if "%errorlevel%" neq "0" echo System config file (%f_vc_cfg%) doesn't consist of options for LINK= & goto build_failed
for /f "tokens=2* delims==" %%A in (lnksopt.tmp) do (echo %%A >> "..\..\intdir\clnk.rsp")

rem Get user options for linker
findstr /b "LINK=" "..\..\configs\xp\%1.cfg" > lnkuopt.tmp
if "%errorlevel%" neq "0" echo System config file (..\..\configs\xp\%1.cfg) doesn't consist of options for LINK= & goto build_failed
for /f "tokens=2* delims==" %%A in (lnkuopt.tmp) do (echo %%A >> "..\..\intdir\clnk.rsp")

%f_vc_path%\link @"..\..\intdir\clnk.rsp"

if "%errorlevel%" == "0" echo Linker produced "..\..\intdir\%1\%1.exe" successfully 

findstr /b "POSTBUILD=" "..\..\configs\xp\%1.cfg" > postbuopt.tmp
for /f "tokens=2* delims==" %%A in (postbuopt.tmp) do (call %%A %CD%)
